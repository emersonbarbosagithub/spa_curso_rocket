...
class Instrutor extends Component {
    ...
    cadastraInstrutor(name, email, curso) {
        ...
        curso_selecionado.then(
            response => {
                return fetch(`https://nameless-reaches-68184.herokuapp.com:${process.env.PORT}/instrutores/`,{
                    method: "POST",
                    body: JSON.stringify({
                        name: name, 
                        email: email, 
    ...
    pegaDados(id) {
        if (id === undefined) {
            ...
            return fetch(`https://nameless-reaches-68184.herokuapp.com:${process.env.PORT}/instrutores/` + id)
                .then(response => { return response.json(); })
                .then(data =>{
