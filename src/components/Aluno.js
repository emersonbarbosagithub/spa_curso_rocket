...
class aluno extends Component {
    ...
    componentDidMount() {
        fetch('https://nameless-reaches-68184.herokuapp.com/turmas/')
    ...
    pegaDadosturmaId(id) {
        return fetch('https://nameless-reaches-68184.herokuapp.com/turmas/' + id)
                    .then(response => { return response.json(); })
                    .then(data =>{
    ...
    pegaDados(id) {
        if (id === undefined) {
            ...
            return fetch('https://nameless-reaches-68184.herokuapp.com/alunos/' + id)
                .then(response => { return response.json(); })
                .then(data =>{
    ...
    render () {
        ...
        const alunos = this.state.alunos.map((aluno) => {
            for (let i=0; i < this.state.alunos.length; i++) {
                if (this.state.alunos[i].turmas[0] === this.turmas[i]._id){
